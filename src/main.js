...
window.addEventListener("load", () => {
  ...
  if (!cursorEl && sceneEl) {
    ...
    cursorEl.setAttribute(
      "material",
      "shader: flat; opacity: 0.8; transparent: true",
    );
  ...
  console.log("✅ XR Variables declared:", {
    xrSession,
    xrRefSpace,
    hitTestSource,
  });
  ...
  function removeObjectFromScene(objEl) {
    ...
    // Remove from GameManager's spawnedObjects array
    if (window.GameManager) {
      window.GameManager.removeSpawnedObject(objEl);
    }
  ...
  function checkTrashcanCollisions() {
    ...
    trashcans.forEach((trashcan) => {
      ...
      const objectsToCheck = [
        ...(window.GameManager ? window.GameManager.getSpawnedObjects() : []),
      ].filter((obj) => obj && !obj.classList.contains("trashcan"));
    ...
    items.forEach((item, index) => {
      ...
      btn.addEventListener("mouseenter", () => {
        ...
        if (icon)
          icon.setAttribute(
            "animation",
            "property: rotation; to: 25 385 0; dur: 800; easing: easeInOutQuad",
          );
      });
      ...
      if (item.type === "gltf") {
        ...
        icon.setAttribute(
          "material",
          `color: ${item.color}; metalness: 0.5; roughness: 0.1`,
        );
      }
  ...
  startBtn.onclick = async () => {
    ...
    // 1. Hide landing page and activate AR mode
    if (landingPage) {
    ...
    try {
      // 3. DEMANDER LA SESSION AR IMMÉDIATEMENT (Hors du setTimeout)
      console.log("🔍 Requesting immersive-ar session...");
      if (debugEl) debugEl.textContent = "🎯 Demande session AR...";
      ...
      if (debugEl) debugEl.textContent = "✅ Session AR obtenue!";

      // 4. Maintenir le délai visuel du loader après avoir obtenu la session
      setTimeout(async () => {
        if (gameContainer) {
          gameContainer.classList.remove("visible");
          gameContainer.classList.add("hidden");
        ...
        // Assigner la session au renderer de la scène
        sceneEl.renderer.xr.setSession(xrSession);
        console.log("✅ Session set to renderer");
        ...
        // On force la création des manettes directement via le moteur XR (Three.js)
        window.leftController = sceneEl.renderer.xr.getController(0);
        window.rightController = sceneEl.renderer.xr.getController(1);

        // TRÈS IMPORTANT : Il faut les ajouter physiquement à la scène pour que leur position s'actualise
        sceneEl.object3D.add(window.leftController);
        sceneEl.object3D.add(window.rightController);

        console.log("✅ Manettes natives reconnectées pour les lasers !");
  ...
  function xrLoop(time, frame) {
    ...
    if (ses) {
      ...
      let isMenuBtnPressed = false;
      let isBrewBtnPressed = false;

      // Checking Input Sources (On parcourt les manettes)
      // Checking Input Sources (On parcourt les manettes)
                for (const source of ses.inputSources) {
                    if (!source.gamepad) continue;

                    // Gâchette principale (Trigger = index 0)
                    if (source.gamepad.buttons[0] && source.gamepad.buttons[0].pressed) {
                        isAnyBtnPressed = true;
                    }

                    // Sur Quest 3 : Y (gauche) et B (droite) = Bouton numéro 5
                    const btnY_ou_B = source.gamepad.buttons[5];
                    
                    if (btnY_ou_B && btnY_ou_B.pressed) {
                        if (source.handedness === 'left') {
                            isMenuBtnPressed = true;
                            updateARDebug('Y pressé ! (Menu)');
                        } else if (source.handedness === 'right') {
                            isBrewBtnPressed = true;
                            updateARDebug('B pressé ! (Café)');
                        }
                    }
                }

      // --- GESTION DES CLICS LASER ---
      window.isAnyBtnPressed = isAnyBtnPressed;
      if (!isAnyBtnPressed) {
        window.uiClickLock = false;
      }

      // --- GESTION DU VR STORE (INVENTAIRE - MAIN GAUCHE) ---
      if (isMenuBtnPressed) {
        if (!menuToggleLock) {
          menuToggleLock = true;
          if (inventoryEntity) {
      ...
      // --- GESTION DE LA MACHINE À CAFÉ (MAIN DROITE) ---
      if (isBrewBtnPressed && !coffeeMachineLock) {
        ...
        if (rightCtrl) {
          ...
          const spawned = window.GameManager
            ? window.GameManager.getSpawnedObjects()
            : [];
    ...
    const handleControllerInteraction = (controller) => {
      ...
      const isMenuVisible =
        inventoryEntity && inventoryEntity.getAttribute("visible");
      ...
      if (inventoryEntity && inventoryEntity.object3D) {
        inventoryEntity.object3D.traverse((child) => {
          if (
            child.el &&
            child.el.classList.contains("clickable") &&
            child.isMesh
          ) {
            buttons.push(child);
          }
      ...
      if (intersects.length > 0) {
        ...
        if (window.isAnyBtnPressed && !window.uiClickLock) {
          ...
          // Otherwise it's a spawn button
            ...
            spawnObject(
              el.dataset.spawnType,
              el.dataset.spawnColor,
              el.dataset.spawnModel,
              el.dataset.spawnScale,
            );
          }
        }
      } else {
